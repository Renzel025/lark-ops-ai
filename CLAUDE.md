...
- The `lark-expert` agent and `.cursor/rules/lark-expert.mdc` carry deep Lark API knowledge and this repo's conventions — use the agent for anything touching Lark APIs, cards, tokens, or webhook callbacks. `playwright-expert` covers the Grafana screenshot pipeline.
