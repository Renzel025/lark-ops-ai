...
| Doc | Focus |
|-----|--------|
| [ARCHITECTURE_AND_FLOW.md](ARCHITECTURE_AND_FLOW.md) | Webhook routing, session lifecycle (Mermaid) |
...
- Optional: **Issue Watch** (auto-detect player issues in chat), **Slack huddle**, **Grafana screenshots**, **Bitable deployment cards**, **VC ring**, **severity Major/Minor**, and more
...
Lark cloud  →  HTTPS webhook  →  nginx  →  uvicorn (FastAPI main.py)
                                    ...
                                    lark_logic + p0_logic
                                              ↓
                              Outbound: Lark Open API + Groq + Anthropic + Slack
```
...
ENV_PROFILE=dev bash scripts/run_dev.sh   # merges .env + .env.dev
```
...
| AI | `GROQ_API_KEY`, optional `ANTHROPIC_API_KEY` (Issue Watch) |
...
| Slack | `SLACK_BOT_TOKEN`, `SLACK_SEVERITY_PROMPT_BEFORE_AUTOMATION` |
| VC ring | `P0_VC_RING_ENABLED=1`, `P0_MAJOR_CHECK_PERSON_IDS`, OAuth URLs |
| Ongoing buzz | `P0_ONGOING_DM_BUZZ_*`, severity prompt for Major/Minor |
...
**Mermaid** is a text format for drawing diagrams. Tools like GitHub and some editors can turn it into a picture automatically. **You do not need to learn it** — the sections below explain the same thing in normal English and with a simple ASCII sketch.
...
| **Slack API** | Channel ping, huddle automation (optional) |
...
└───────────────────────────┬─────────────────────────────────┘
                            ...
                            ▼
┌─────────────────────────────────────────────────────────────┐
│  Lark API · Groq · Claude · Slack · lark-forwarder          │
└───────────────────────────┬─────────────────────────────────┘
...
```mermaid
flowchart TD
  WH[POST /lark/webhook] --> UV{type?}
  UV -->|url_verification| CH[Return challenge JSON]
  UV -->|encrypt payload| DEC[AES decrypt]
  DEC --> BG[Background task:\n_process_lark_payload]
  BG --> ET{event_type}
  ET -->|im.message.receive_v1| PM[process_message\nlark_logic.py]
  ET -->|card.action.trigger| CA[handle_lark_card_action\nhandlers.py]
  ET -->|vc.meeting.*| VC[Join/leave/end hooks\nparticipants + session]
  ET -->|OAuth callback| OA[vc_user_oauth]
  PM --> DMH[handle_dm_generate_overview\nif DM overview flow]
  PM --> IW[try_handle_issue_watch\nif detection group]
```
...
```mermaid
flowchart TD
  MSG[Text in incident group] --> P1P{P1 prompt pending?}
  P1P -->|yes| P1H[P1 confirm handlers]
  P1P -->|no| CMD{cancel / end /\ncooldown reset?}
  CMD -->|yes| CMDH[session end/cancel/cooldown]
  CMD -->|no| P0K{p0 keyword?}
  P0K -->|yes + allowed| SP0[start_p0]
  P0K -->|no| P1K{p1 keyword?}
  P1K -->|yes| RP1[P1 confirmation card]
  P1K -->|no| IW[Issue Watch classify]
  IW --> IGN[Ignore — no reply]
```
...
stateDiagram-v2
  [*] --> Idle
  Idle --> P0_Active: p0 / Issue Watch Declare P0
  Idle --> P1_Prompt: p1 keyword
  P1_Prompt --> P1_Active: user confirms meeting
  P1_Prompt --> Idle: Not needed
  P1_Active --> P0_Active: 15m card Declare P0
  P0_Active --> Idle: end / cancel / VC ended
...
5. Optional **Slack severity** DM (Major/Minor) via second Lark app
6. Optional Slack channel ping + huddle queue
7. Schedule **ongoing DM buzz** (5 min Major / 10 min Minor)
8. Schedule graph screenshot (if enabled)
9. Issue Watch: queue suggested overview if declared from alert
...
```mermaid
sequenceDiagram
  participant G as Incident group
  participant B as Bot
  participant D as Duty DM
  participant AI as Groq

  G->>B: p0 declared
  B->>G: VC link (plain text + unfurl)
  B->>D: Green instruction card
  D->>B: paste text + screenshots
  D->>B: Build overview
  B->>AI: generate issue/impact/Chinese
  B->>D: Blue preview card
  D->>B: Send to group
  B->>G: Bilingual overview card
  B->>G: Optional Bitable deployment cards
  B->>D: Clear draft; next queued incident if any
```
...
2. Bot classifies (Claude or keyword rules)
...
ANTHROPIC_API_KEY=...   # or keyword fallback if Claude fails
```
...
### 9.5 Severity Major / Minor (optional second bot)

After P0 declare, the severity bot DMs duty:

- **Major** → Slack huddle automation (if enabled); **5 min** ongoing buzz if meeting still active
- **Minor** → follow-up role cards; **10 min** ongoing buzz
...
Queries **Deployments** + **线上操作** tables; posts cards for rows in the **48h MYT window** (yesterday 00:00 → today 23:59).
...
### 9.8 Slack huddle (optional)

Separate Playwright script on server/VNC — see `scripts/` and the Slack section in `env.example`.
...
sequenceDiagram
  participant D as Duty DM
  participant P as Primary bot (lark-ops-ai)
  participant F as lark-forwarder
  participant OB as Overview bot
  participant Det as Detection group
  participant Br as Broadcast group

  D->>P: Send to group
  P->>Det: Post overview card (primary bot)
  P->>F: POST /post-overview (markdown + broadcast oc_)
  F->>OB: Lark IM API
  OB->>Br: Post overview card (overview bot)
  P->>P: Store primary + broadcast message_id
  Note over D,Br: Edit → Save PATCHes both cards when forwarder returns message_id
```
...
| Claude fails | Anthropic credits; keyword fallback still works for deposit/login patterns |
...
7. **Integrations** — Slack, Bitable, VC ring, recording fan-out, graph screenshot, **broadcast overview forwarder**
...
└── docs/
    ...
    ├── ARCHITECTURE_AND_FLOW.md       ← Mermaid: webhook + session
