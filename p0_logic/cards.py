...
def _help_commands_md() -> str:
    ...
    return (
        ...
        "---\n\n"
        "**⌨️ Type in incident group / 事故群输入**\n"
        "• **em** — end active P0 or P1 session\n"
        "• **pe** — end P0 (also: `p0 end`, `end p0`, `p0 resolved`)\n"
        "• **p1e** — end P1 (also: `p1 end`, `end p1`, `p1 resolved`)\n"
        "• **cm** — cancel meeting (+ optional reason after, e.g. `cm no need yet`)\n"
        "• **cr** — cooldown reset (no new VC)\n"
        "• **yes** / **no** — while P1 “create meeting?” is open (or tap buttons below)\n\n"
        "**⌨️ Type in DM / 私聊输入**\n"
        "• **st** — draft status (also: `status`, `draft`, `check`)\n"
        "• **cl** — clear draft (also: `clear`, `reset`; if preview is open, tap **Cancel** instead)\n"
        "• **coe** — standalone overview, emergency group (no meeting)\n"
        "• **cog** — standalone overview, game group (no meeting)\n"
        "• **pt** — list meeting participants (also: `participants`)\n"
        "• **is Alice in the meeting?** — check one person (replace name)\n"
        "• **h** — show this card (also: `help`, `commands`)\n\n"
