...
# Usage: sudo bash scripts/diagnose_p0_incident_logs.sh [since] [until]
# Example: sudo bash scripts/diagnose_p0_incident_logs.sh '2026-06-08 17:35' '2026-06-08 17:50'
...
journalctl "${JARGS[@]}" | grep -iE \
  'start_p0|graph screenshot|Capturing Grafana|playwright|Issue Watch|issue_watch|scheduling auto capture|skipped —|overview will DM|no Issue Watch alert' \
  || echo "(no matching log lines — widen time window or check service name)"

echo
echo "=== env flags (no secrets) ==="
if [[ -f "$ENV_FILE" ]]; then
  grep -E '^(P0_GRAPH_SCREENSHOT_|P0_ISSUE_WATCH_|P0_SHARED_STATE_DIR|P0_DM_INSTRUCTION)' "$ENV_FILE" \
